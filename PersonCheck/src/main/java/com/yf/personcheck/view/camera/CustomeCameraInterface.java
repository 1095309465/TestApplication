...
public class CustomeCameraInterface implements Camera.PreviewCallback {

    private static final String TAG = "CJT";

    private volatile static CustomeCameraInterface mCameraInterface;
    ...
    //获取CameraInterface单例
    public static synchronized CustomeCameraInterface getInstance() {
        if (mCameraInterface == null)
            synchronized (CustomeCameraInterface.class) {
                if (mCameraInterface == null)
                    mCameraInterface = new CustomeCameraInterface();
            }
    ...
    private CustomeCameraInterface() {
     ...
     * open Camera
     */
    void doOpenCamera(CustomeCameraInterface.CameraOpenOverCallback callback) {
        ...
        if (mCamera != null) {
            try {
                ...
                if(onPreviewSizeListener!=null){
                    onPreviewSizeListener.getPreviewSize(previewSize);
                }
    ...
    public void takePicture(final CustomeCameraInterface.TakePictureCallback callback) {
        if (mCamera == null) {
            return;
        }
        switch (cameraAngle) {
    ...
    //启动录像
    public void startRecord(Surface surface, float screenProp, CustomeCameraInterface.ErrorCallback callback) {
        mCamera.setPreviewCallback(null);
    ...
    //停止录像
    public void stopRecord(boolean isShort, CustomeCameraInterface.StopRecordCallback callback) {
        if (!isRecorder) {
            return;
        }
        if (mediaRecorder != null) {
    ...
    public void handleFocus(final Context context, final float x, final float y, final CustomeCameraInterface.FocusCallback callback) {
        if (mCamera == null) {
            return;
        }
    ...
    public OnPreviewSizeListener getOnPreviewSizeListener() {
        return onPreviewSizeListener;
    }

    public void setOnPreviewSizeListener(OnPreviewSizeListener onPreviewSizeListener) {
