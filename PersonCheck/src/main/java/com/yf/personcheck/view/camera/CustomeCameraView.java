package com.yf.personcheck.view.camera;
...
public class CustomeCameraView extends FrameLayout implements CustomeCameraInterface.CameraOpenOverCallback, SurfaceHolder
        .Callback, CameraView {
    ...
    private CustomeCaptureLayout mCaptureLayout;
    ...
    public CustomeCameraView(Context context) {
        this(context, null);
    }

    public CustomeCameraView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public CustomeCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView() {
        ...
        View view = LayoutInflater.from(mContext).inflate(R.layout.custome_camera_view, this);
        ...
        mCaptureLayout = (CustomeCaptureLayout) view.findViewById(R.id.capture_layout);
...
//                mCaptureLayout.setTextWithAnimation("录制时间过短");
    ...
    @Override
    public void cameraHasOpened() {
        CustomeCameraInterface.getInstance().doStartPreview(mVideoView.getHolder(), screenProp);
    }

    //生命周期onResume
    public void onResume() {
        resetState(TYPE_DEFAULT); //重置状态
        CustomeCameraInterface.getInstance().registerSensorManager(mContext);
        CustomeCameraInterface.getInstance().setSwitchView(mSwitchCamera, mFlashLamp);
    ...
    public void onPause() {
        ...
        CustomeCameraInterface.getInstance().isPreview(false);
        CustomeCameraInterface.getInstance().unregisterSensorManager(mContext);
    }
    ...
    public void surfaceCreated(SurfaceHolder holder) {
        new Thread() {
            @Override
            public void run() {
                CustomeCameraInterface.getInstance().doOpenCamera(CustomeCameraView.this);
            }
        }.start();
    }
    ...
    public void surfaceDestroyed(SurfaceHolder holder) {
        CustomeCameraInterface.getInstance().doDestroyCamera();
    }
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (event.getPointerCount() == 2) {
                    ...
                    if ((int) (result - firstTouchLength) / zoomGradient != 0) {
                        firstTouch = true;
                        machine.zoom(result - firstTouchLength, CustomeCameraInterface.TYPE_CAPTURE);
                    }
    ...
    public void setSaveVideoPath(String path) {
        CustomeCameraInterface.getInstance().setSaveVideoPath(path);
    }
    ...
    public void setMediaQuality(int quality) {
        CustomeCameraInterface.getInstance().setMediaQuality(quality);
    }
    ...
    public void playVideo(Bitmap firstFrame, final String url) {
        videoUrl = url;
        CustomeCameraView.this.firstFrame = firstFrame;
    ...
    public void setCaptureListener(CaptureListener clickListener) {
        this.captureListener = clickListener;
    }

    public void setCustomeListener(CustomeListener clickListener) {
        this.customeListener = clickListener;
    }
